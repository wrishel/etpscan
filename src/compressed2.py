...
class ScanControl:
    ...
    def timeoutfunc(self):
        ...
        if return_code is not None:
            ...
            if scan_count > 0:
                new_label = "Scanned from %d to %d (%d sides or %d pages) pct %s by %s" % (
                self.scan_start, self.highwater, scan_count, (scan_count / 2), self.pct_entry.get_text(),
                self.who_entry.get_text())
